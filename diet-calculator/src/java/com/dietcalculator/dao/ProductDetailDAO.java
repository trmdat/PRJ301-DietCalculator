...
public class ProductDetailDAO {
    ...
    public boolean createProductDetail(String detailID, String saleoffID, String productID, int percentoff, float priceoff, String gift){
     ...
     public boolean updateProductDetail(String saleoffID, String productID, int percentoff, float priceoff, String gift){
        int row = 0;
        String sql ="update ProductDetail SET saleoffID = ?, productID =?, percentoff =?, priceoff =?, gift =?";
        try{
            Connection conn = DBUtils.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql);
